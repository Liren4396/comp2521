...
Graph GraphMST(Graph g) {
    ...
    PQ pq = PQNew();
    Edge **new = (Edge **)malloc(sizeof(Edge *) * g->nV);
    for (int i = 0; i < g->nV; i++) {
        new[i] = (Edge *)malloc(sizeof(Edge) * g->nV);
        for (int j = i + 1; j < g->nV; j++) {
            new[i][j].v = i;
            new[i][j].w = j;
            new[i][j].weight = g->edges[i][j];
            if (i != j) {
                PQInsert(pq, new[i][j]);
            }
        }
    }
    //sort edges(G) by weight in ascending order
    int count = 0;
    while (! PQIsEmpty(pq)) {
        Edge e = PQExtract(pq);
        if (e.weight == 0.000000) {
            continue;
        }
        if (GraphInsertEdge(MST, e)) {
            count++;
        }
